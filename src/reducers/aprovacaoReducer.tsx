...
const aprovacaoReducer = (state:any = INTIAL_STATE, action:any) => {
  switch (action.type) {
    case FETCH_APROVACAO:
      return { ...state, aprovacaoList: action.payload.response.data, filter: action.payload.filter }
    ...
    case FETCH_DOCUMENTO_CONFIRMACAO:
      ...
      const novosDocumentos = state.aprovacaoDetail.documentos.map( (doc:any) => {
      ...
      const aprovacaoDetail = { ...state.aprovacaoDetail, documentos: novosDocumentos }
